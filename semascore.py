...
Original file is located at
    https://colab.research.google.com/drive/143lZphDC9-lzLojUsbH8IXC6x9nhO2lI
"""
...
def get_hyp_embeddings_v1(aligned_v1, hyp_embedding, hyp_tokens):
    ...
    for j in range(len(aligned_v1)):
        # Reset the word for each element in aligned_v1
        word = ''
        start = k + 1
...
# Load the Excel file
input_file = "/content/common_500_llama_lm_new.xlsx"
df = pd.read_excel(input_file)

# Ensure the column names match the actual column names in your Excel file
ground_truth_column = 'reference'  # column name for ground truth sentences
hypothesis_column = 'hypothesis'   # column name for hypothesis sentences
...
for index, row in df.iterrows():
    ...
    # Calculate the SEMA score for the current pair
    score = generate_sema_score(ground_truth, hypothesis)

    # Append the score to the results list
    sema_scores.append(score)

# Add the SEMA scores to a new column in the DataFrame
df['sema_score'] = sema_scores

# Optionally, save the results back to a new Excel file
output_file = "common_500_llama_lm_sema_score.xlsx"
df.to_excel(output_file, index=False)

print("calculations complete. Results saved to", output_file)